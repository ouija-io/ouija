...
function S3Deploy(config) {
  ...
  var path = this._namespace + '/' + this._repo + '/';
...
S3Deploy.prototype._uploadFiles = function(fileNames, cb) {
  ...
  async.each(fileNames, function(fileName, done) {
    var filePath = self._source + '/' + fileName;
    ...
    fs.readFile(filePath, function(err, data) {
      ...
      var tasks = [
        _.bind(self._uploadFile, self, type, data, self._keys.tag + fileName)
      ];

      if (self._tagData.isLatest) {
        tasks.push(_.bind(self._uploadFile, self, type, data,
...
S3Deploy.prototype._uploadFile = function(type, data, dest, cb) {
  var params = {
    Bucket: this._bucket,
    Key: dest,
    Body: data,
